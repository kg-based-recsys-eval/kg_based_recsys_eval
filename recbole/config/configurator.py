...
class Config(object):
    ...
    def _load_internal_config_dict(self, model, model_class, dataset):
        ...
        knowledge_base_on_dataset_init = os.path.join(quick_start_config_path, 'knowledge_base_' + dataset + '.yaml') ## NEW
        #knowledge_base_on_ttr_init = os.path.join(quick_start_config_path, 'knowledge_base_ttr.yaml') ## NEW
        ...
        if self.internal_config_dict['MODEL_TYPE'] == ModelType.GENERAL:
            ...
            if dataset in ('ml-1m', 'ttr', 'amazon-books'): ## NEW
                self._update_internal_config_dict(knowledge_base_on_dataset_init) ## NEW
